...
def combine_verb(verb, pos_tags):
    for i in range(len(pos_tags)):
        ...
        if t1[0] == verb:
            if len(pos_tags) > i + 1 and pos_tags[i+1][0] == 'not': 
    ...
    for label in labels:
        ...
        for tag in pos_tags:
            ...
            if is_in_label:
                ...
                label_dict[tag[1] + '_' + str(i)] = tag[0]
                # if tag[0] == verb or tag[1][0:2] == 'VB':
                #     label_dict['V' + str(i)] = tag[0]
                # else:
                #     label_dict[tag[1] + str(i)] = tag[0]
            is_in_label = False
            i = i + 1
        labels_list.append(label_dict)
    return labels_list

def preprocess_tags(labels, pos_tags):
    rst = []
    for label in labels:
        tag_list = []
        # Ignore if less than 3 elements in labels
        ...
        for tag in pos_tags:
            if tag[1] == '.':
                continue
            if tag[0] in ['Who', 'Whose', 'What', 'Where', 'When', 'Which', 'Why', 'How']:
                tag_list.append(('W', tag[0]))
                continue
            # Check if any label contains tag, label could be a phrase.
            current_value = ''
            for value in label.values():
            ...
            if is_in_label:
                # Get key by value in dict
                key = list(label.keys())[list(label.values()).index(current_value)]
                if last_value != label[key]:
                    tag_list.append((key, label[key]))
                last_value = label[key]
            else:
...
def get_tag_seq(pos_tag):
    return [t[0] for t in pos_tag]
